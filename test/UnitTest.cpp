...
int main() {
  ...
  string PATTERN[4] = {"FATAL_CORE_DUMP_CORRUPT_BUFFER_9999", "USER_SESSION_ERR_AUTH_CODE_[0-9]{4}",
  ...
  for (int num = 0; num < 5; num++) {
    ...
    for (Log_Query res : results) {
      if (num == 0) {
        ...
        if (((res.machine_id == 3 || res.machine_id == 8 || res.machine_id == 9) &&
             res.content != expected[3]) ||
            (res.machine_id != 3 && res.machine_id != 8 && res.machine_id != 9 &&
